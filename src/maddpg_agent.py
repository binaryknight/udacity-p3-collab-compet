...
class MAgent():
    ...
    def step(self, states, actions, rewards, next_states, dones, t_step):
        ...
        for idx, agent in enumerate(self.agents):
            agent.memory.add(states[[idx]], actions[[idx]], rewards[idx],
                             next_states[[idx]], dones[idx], states,
                             actions, next_states)
    ...
    def learn(self, experiences, gamma, train_agent):
        pdb.set_trace()
        states, actions, rewards, next_states, dones, observations, observed_actions, next_observations = experiences
        # ---------------------------- update critic -------------------------#
        ...
        Q_targets_next = train_agent.critic_target(obs_next,
                                                   actions_next)
        ...
        train_agent.soft_update(train_agent.critic_local,
                                train_agent.critic_target, TAU)
        train_agent.soft_update(train_agent.actor_local,
                                train_agent.actor_target, TAU)
